...
* Note........: 160622 bug fix. Earlier Std IVF was coded as frozen unknown   ;
* Note........: 160622 resolved testis earlier set to M missing               ;
*-----------------------------------------------------------------------------;
* Data used...:                                                               ;
...
/*
proc sort data=chrontic1 out=slask1 nodupkey;by lopnr;run;
proc sort data=mbr1(keep=lopnr_barn byear) out=slask2 nodupkey;by lopnr_barn byear;run;
proc sql;
  create table slask3 as select b.*
  from slask1 as a join slask2 as b on a.lopnr=b.lopnr_barn;
*/

*---------------------------------------------------------------------------------------------------------------;
* Added code below 2016-05-08. A transient tic disorder after a diag of chronic invalidte the chronic diagnosis ;
* As far as I can see there is no TD diagnosis before ICD10 in Sweden, that is 1997                             ;
*                                                                                                               ;
* RULE:                                                                                                         ;
* Use all F42 as OCD. For TIC                                                                                   ;
* a) Use all F95.1 and F95.2 - but not if a transient tic (F95.0) within one year                               ;
* b) Use F95.8 and F95.9 (other) if two in a row and dont consider any where there is a F95 within a year       ;
*                                                                                                               ;
*---------------------------------------------------------------------------------------------------------------;

data transtic(keep=lopnr diag_dat) chrontic1 othertic1 chrontic2;
  set dd3;
  if (icd='8' and substr(left(diag),1,5)='306,6') or (icd='9' and diag='307C') then output chrontic1;
  if icd='10' then do;
    if diag='F950' then output transtic;
    if diag in ('F951','F952') then output chrontic2;
    if diag in ('F958','F959') then output othertic1;
  end;
run;

* Now, transient tic within the same year as a chronic in-validates the chronic. Thus, records in chrontic2 and othertic2 are not valid;
data chrontic2;
  merge chrontic1(in=chrontic)
        transtic(in=transtic rename=(diag_dat=trans_dat));
  by lopnr;

  if chrontic and transtic then do;
    if 0 GE trans_dat-diag_dat LE 365 then output;
  end;
run;
data othertic2;
  merge othertic1(in=othertic)
        transtic(in=transtic rename=(diag_dat=trans_dat));
  by lopnr;

  if othertic and transtic then do;
    if 0 GE trans_dat-diag_dat LE 365 then output;
  end;
run;

*-- Now prepare the DD3 dataset: Select codes to consider for TIC and OCD;
data dd3_ocd dd3_tic1 dd3_tic_other1;
  set dd3;
  if icd='10' then do;
    if substr(left(diag),1,3)='F42' then output dd3_ocd;
    else if substr(left(diag),1,4)='F951' or
            substr(left(diag),1,4)='F952' then output dd3_tic1;
    else if substr(left(diag),1,4)='F958' or
            substr(left(diag),1,4)='F959' then output dd3_tic_other1;
  end;
run;

*-- Remove the TICs considered transient;
data dd3_tic2;
  merge dd3_tic1(in=tic1)
        transtic(in=transtic rename=(diag_dat=trans_dat));
  by lopnr;
  if tic1 and not transtic then output;
  else if tic1 and transtic then do;
    if 0 GE trans_dat-diag_dat LE 365 then delete;
    else output;
  end;
run;

*-- Remove the other TICs considered transient;
data dd3_tic_other2;
  merge dd3_tic_other1(in=tic1)
        transtic(in=transtic rename=(diag_dat=trans_dat));
  by lopnr;
  if tic1 and not transtic then output;
  else if tic1 and transtic then do;
...
*-- Now, for the other TICs, only consider individuals with two diagnosis of other (not invalidated by a transient within a year);
proc summary data=dd3_tic_other2 nway;
  var lopnr;
  class lopnr;
  output out=dd3_tic_other3(keep=lopnr n_other) n=n_other;
run;

data dd3_tic_other4 slask(label='Only one tic disorder - Disqualified');
  merge dd3_tic_other2
        dd3_tic_other3(keep=lopnr n_other);
  by lopnr;

  if n_other>1 then output dd3_tic_other4;
  else output slask;
run;


*-- Finally, stack the datasets for OCD and TICS;
data dd3_final(label='All diagnosis of valid OCD and TICS');
  drop _c_;retain _c_ 0;
  set dd3_ocd
      dd3_tic2
      dd3_tic_other4 end=eof;
  by lopnr;
  if first.lopnr then _c_=_c_+1;

  if eof then put 'NOTE: There are a total of ' _c_ 'cases of OCD+CT';
run;



*---------------------------------------------------------------------------------------------------------------------------;
* By now the diagnostic codes have been selected (above). Here I join diagnosis date with death and emigration and sorting  ;
* the diagnosis into OCD, TICS and Tourette (part of TICS).                                                                 :
* Calculating ge at diagnosis                                                                                               ;
*---------------------------------------------------------------------------------------------------------------------------;
...
*  drop met metod1 metod2;
  ...
  *-- 2016-06-22. Bug fix. Added 1,2 to: if met in (1,2,3,4,5) then frozen=0 ;
  *-- 2016-06-22. Bug fix. met=7 was coded both as testis=0 and testis=.M. Now testis=0;
  met=input(metod,8.);
  if met in (1,2,6) then ivf_icsi=1;
  else if met in (3,4,5,7,8,9,10) then ivf_icsi=2;
  else if met in (0) then ivf_icsi=.M;
  if met in (1,2,3,4,5) then frozen=0;
  ...
  else if met in (9,0) then testis=.M;
...
data ivfdata(label='IVF data from SOS and Q-IVF');
  ...
  *-- 2016-06-22. If trt variable dont indicate sperm surgery, then assume no surgery;
  if trt not in (4,5) then testis=0;
run;
