...
proc sort data=mbr1(keep=lopnr_barn child_bdat) out=tmp(rename=(lopnr_barn=lopnr));by lopnr_barn;run;

data npr4(keep=lopnr_barn outcome diag_dat diag_age)
     chk_diag_age(keep=lopnr_barn outcome diag_dat diag diag_age);
...
*  attrib ott length=3 label="OCD/TICS/TD" length=3
  ...
  ;
  merge tmp npr3(in=npr3);by lopnr;
...
data ana0;
  attrib entry   length=6  label='Age at entry'
         ...
         pt      length=3  label='Pre-Term' format=yesno.
  ;
  merge mbr3(in=mbr3)
        npr4(in=npr);
  ...
  *-- Require that in MBR and born before 1st Jan 2011;
  if mbr3 and child_bdat < '01JAN2011'd then do;
    *-- Cohort entry;
    ...
    if gage le .z then pt=.u;
    else if gage < 37 then pt=1;
    else pt=0;
...
data check;set ana0(obs=200);run;

%tor(data=check, dname=psy1502_ana0, rfile=psy1502_ana0.R,
     class=outcome trt byear sex art paritet singleton gage  ivf_icsi frozen surg,
