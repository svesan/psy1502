...
* Note........: 160427 bug fix                                                ;
* Note........: 160610 added male_inf variable                                ;
*-----------------------------------------------------------------------------;
...
proc sql;
  ...
  select a.outcome, a.lopnr_barn, a.entry, a.exit, a.event,
         ...
         b.infert_rel_diag, b.infcat, b.male_inf
  from tmp1 as a
...
data _null_;
  ...
  if entry=exit then do; c=c+1; put lopnr_barn= 'ERROR: Entry = exit';     end;
  if entry>exit then do; c=c+1; put lopnr_barn= 'ERROR: Entry after exit'; end;
  if entry<1 then do; c=c+1; put lopnr_barn= 'ERROR: Entry before age 1';  end;
  if entry>29 then do; c=c+1; put lopnr_barn= 'ERROR: Entry after age 29'; end;
...
%tor(data=coxdsn, dname=coxdsn, rfile=coxdsn.R,
     ...
     class=outcome sex group trt trtb testis ivf_icsi frozen mat_cat pat_cat 
           mat_phist pat_phist preterm twin infert_rel_diag infcat male_inf);
...
*-- End of File --------------------------------------------------------------;
/*
filename saspgm '/home/svesan/ki/ABS/mep/sasproj/PSY/PSY1502/saspgm'; * libname onjold;

proc upload incat=work.formats outcat=work.formats;run;
proc upload data =work.coxdsn  out   =work.coxdsn ;run;
